...
- [ ] 상품 상세 페이지
  - [ ] **레이아웃 구조**
    - 상단 섹션: 상품 이름, 가격, 재고 상태 표시
    - 중단 섹션: 상품 설명, 카테고리 정보 표시
    - 우측 고정 섹션: 장바구니 추가 UI (수량 선택, 장바구니 추가 버튼)
    - 하단 섹션: 등록일 및 수정일 표시
  - [ ] **기술 구현**
    - Server Action으로 상품 정보 조회 (`actions/get-product.ts`)
    - 상품 이미지 갤러리 (MVP: 플레이스홀더 이미지 사용)
    - 수량 선택 기능 (최소 1개, 최대 재고 수량)
    - 장바구니 추가 기능 (재고 부족 시 처리)
    - 로딩 상태 및 에러 처리 (상품 미존재 시 404)
    - 반응형 디자인
